...
source: "https://javaguide.cn/java/concurrent/java-lock.html"
author:
  - "[[Guide]]"
published: 2026-06-15
created: 2026-07-06
description: "Java 锁机制系统梳理：从互斥锁、读写锁、自旋锁到 synchronized、ReentrantLock、AQS、StampedLock，讲清锁分类、实现原理、版本差异与选型建议。重点围绕 一把锁到底保护什么？、互斥锁：同一时刻只允许一个线程进入、读写锁：读读共享，写操作独占、自旋锁：不阻塞，先原地等一会儿等核心内。"
tags:
  - "clippings"
---
...
这篇文章先把锁的坐标系立起来，再看 Java 里常用锁工具怎么落地。关于悲观锁、乐观锁和 CAS 的细节，站内已经有两篇文章详细介绍： [乐观锁和悲观锁详解](https://javaguide.cn/java/concurrent/optimistic-lock-and-pessimistic-lock.html) 、 [CAS 详解](https://javaguide.cn/java/concurrent/cas.html) 。本文只保留必要上下文，重点放在“锁体系怎么串起来”。
...
很多并发工具都建立在 AQS 之上，比如 `ReentrantLock` 、 `Semaphore` 、 `CountDownLatch` 、 `ReentrantReadWriteLock` 。如果想把队列、 `state` 、CAS 和阻塞唤醒这条线继续拆开，可以接着看 [AQS 详解](https://javaguide.cn/java/concurrent/aqs.html) 和 [从ReentrantLock的实现看AQS的原理及应用](https://javaguide.cn/java/concurrent/reentrantlock.html) 。
...
两个线程分别按 `A -> B` 和 `B -> A` 的顺序加锁，很容易形成死锁。多把锁同时使用时，要给资源排一个全局稳定顺序。死锁的完整介绍可以看 [死锁详解](https://javaguide.cn/cs-basics/operating-system/dead-lock.html) 。
