---
title: "SQL常见面试题总结（3）"
source: "https://javaguide.cn/database/sql/sql-questions-03.html#sql-%E7%B1%BB%E5%88%AB%E9%AB%98%E9%9A%BE%E5%BA%A6%E8%AF%95%E5%8D%B7%E5%BE%97%E5%88%86%E7%9A%84%E6%88%AA%E6%96%AD%E5%B9%B3%E5%9D%87%E5%80%BC-%E8%BE%83%E9%9A%BE"
author:
  - "[[Guide]]"
published: 2026-05-18
created: 2026-07-06
description: "SQL常见面试题总结第三篇，深入讲解聚合函数COUNT、SUM、AVG、MAX、MIN的使用，以及GROUP BY分组、HAVING过滤、截断平均值计算等进阶技巧。重点围绕 聚合函数、分组查询、嵌套子查询、合并查询、数据库基础 等内容展开。结合 JavaGuide 知识体系梳理 数据库 / SQL / Sql Que。"
tags:
  - "clippings"
---
...
| id | exam\_id | tag | difficulty | duration | release\_time |
| --- | --- | --- | --- | --- | --- |
| 1 | 9001 | SQL | hard | 60 | 2020-01-01 10:00:00 |
| 2 | 9002 | 算法 | medium | 80 | 2020-08-02 10:00:00 |

示例数据： `exam_record` （uid 用户 ID, exam\_id 试卷 ID, start\_time 开始作答时间, submit\_time 交卷时间, score 得分）

| id | uid | exam\_id | start\_time | submit\_time | score |
| --- | --- | --- | --- | --- | --- |
...
| tag | difficulty | clip\_avg\_score |
| --- | --- | --- |
| SQL | hard | 81.7 |
...
**思路一：** 要找出高难度 sql 试卷，肯定需要联 examination\_info 这张表，然后找出高难度的课程，由 examination\_info 得知，高难度 sql 的 exam\_id 为 9001，那么等下就以 exam\_id = 9001 作为条件去查询；
...
| total\_pv | complete\_pv | complete\_exam\_cnt |
| --- | --- | --- |
| 10 | 7 | 2 |
...
示例数据 exam\_record 表（uid 用户 ID, exam\_id 试卷 ID, start\_time 开始作答时间, submit\_time 交卷时间, score 得分）：
...
| id | uid | question\_id | submit\_time | score |
| --- | --- | --- | --- | --- |
...
| submit\_month | month\_q\_cnt | avg\_day\_q\_cnt |
| --- | --- | --- |
| 202108 | 2 | 0.065 |
| 202109 | 3 | 0.100 |
| 2021 汇总 | 5 | 0.161 |
...
> 牛客已经采用最新的 Mysql 版本，如果您运行结果出现错误：ONLY\_FULL\_GROUP\_BY，意思是：对于 GROUP BY 聚合操作，如果在 SELECT 中的列，没有在 GROUP BY 中出现，那么这个 SQL 是不合法的，因为列不在 GROUP BY 从句中，也就是说查出来的列必须在 group by 后面出现否则就会报错，或者这个字段出现在聚合函数里面。
...
| uid | incomplete\_cnt | complete\_cnt | detail |
| --- | --- | --- | --- |
...
| id | uid | nick\_name | achievement | level | job | register\_time |
| --- | --- | --- | --- | --- | --- | --- |
...
现有试卷作答记录表 exam\_record（uid 用户 ID, exam\_id 试卷 ID, start\_time 开始作答时间, submit\_time 交卷时间, score 得分）：
...
题目练习表 practice\_record（uid 用户 ID, question\_id 题目 ID, submit\_time 提交时间, score 得分）：
...
现有用户信息表 user\_info（uid 用户 ID，nick\_name 昵称, achievement 成就值, level 等级, job 职业方向, register\_time 注册时间）：
...
试卷信息表 examination\_info（exam\_id 试卷 ID, tag 试卷类别, difficulty 试卷难度, duration 考试时长, release\_time 发布时间）：
...
试卷作答记录表 exam\_record（uid 用户 ID, exam\_id 试卷 ID, start\_time 开始作答时间, submit\_time 交卷时间, score 得分）：
...
题目练习记录表 practice\_record（uid 用户 ID, question\_id 题目 ID, submit\_time 提交时间, score 得分）：
...
| uid | exam\_id | start\_time | submit\_time | score |
| --- | --- | --- | --- | --- |
...
| uid | question\_id | submit\_time | score |
| --- | --- | --- | --- |
...
| uid | act\_month\_total | act\_days\_2021 | act\_days\_2021\_exam |
| --- | --- | --- | --- |
| 1006 | 3 | 4 | 1 |
| 1001 | 2 | 2 | 1 |
| 1005 | 1 | 1 | 1 |
