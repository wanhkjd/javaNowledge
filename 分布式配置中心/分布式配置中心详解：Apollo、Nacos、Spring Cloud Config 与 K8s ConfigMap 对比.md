...
source: "https://javaguide.cn/distributed-system/distributed-configuration-center.html"
author:
  - "[[Guide]]"
published: 2026-06-25
created: 2026-07-06
description: "分布式配置中心原理与选型详解，涵盖 Apollo、Nacos、Spring Cloud Config、Kubernetes ConfigMap 的架构差异、配置推送机制、灰度发布、高可用设计和面试高频考点。重点围绕 为什么要用配置中心？、常见的配置中心有哪些？如何选择？、配置中心、注册中心与 K8s ConfigMa。"
tags:
  - "clippings"
---
## 为什么要用配置中心？
...
从分布式系统视角看，配置中心属于典型控制面：它负责决定“当前应该使用哪一份配置”，客户端负责拉取、缓存、监听和刷新。配置发布、灰度、回滚、客户端本地快照，处理的都是集中决策和客户端容灾之间的取舍。这个取舍可以和 [分布式协调详解](https://javaguide.cn/distributed-system/protocol/centralized-and-decentralized.html) 放在一起理解。

配置中心还经常和 [RPC](https://javaguide.cn/distributed-system/rpc/) 以及 [API 网关](https://javaguide.cn/distributed-system/api-gateway.html) 一起出现：RPC 框架需要拿到服务地址、超时、重试、降级等治理配置，网关也可能依赖配置中心做动态路由和灰度规则。它们解决的问题不同，但在真实微服务体系里通常会连在同一条链路上。
