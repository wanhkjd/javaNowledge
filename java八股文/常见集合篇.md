...
> 下图为 HashMap 底层全景（JDK 1.8）：左侧是"数组 + 链表 + 红黑树"的存储结构——数组按 hash 定位桶，冲突时链表拉链、过长转红黑树、无冲突则单节点直存；下方是寻址算法、扩容机制、树化/退化、1.7→1.8 版本演进四大核心机制。可对照本节各小节理解。

![[java八股文/资源/图-HashMap底层结构.png]]
