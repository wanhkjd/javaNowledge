...
> 下图为这三大高频模式的结构与应用对比：① 工厂方法（简单工厂 → 工厂方法 → 抽象工厂的演进，核心是解耦对象创建）；② 策略模式（Context 持有抽象策略，多个具体策略实现，运行时切换以消除 if-else）；③ 责任链模式（请求沿处理者链逐级传递，类比拦截器 / Filter）。底部给出「工厂 + 策略」在登录场景的经典组合，可对照下文各节理解。

![[java八股文/资源/图-设计模式三大类.png]]
