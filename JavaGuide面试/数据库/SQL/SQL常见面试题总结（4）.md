---
title: "SQL常见面试题总结（4）"
source: "https://javaguide.cn/database/sql/sql-questions-04.html"
author:
  - "[[Guide]]"
published: 2026-05-18
created: 2026-07-06
description: "SQL常见面试题总结第四篇，详解MySQL 8.0窗口函数ROW_NUMBER、RANK、DENSE_RANK、NTILE、LAG、LEAD等的用法和应用场景。重点围绕 专用窗口函数、聚合窗口函数、写在最后、数据库基础、SQL 等内容展开。结合 JavaGuide 知识体系梳理 数据库 / SQL / Sql Que。"
tags:
  - "clippings"
---
...
FROM table;
```
7. `FIRST_VALUE()` 和 `LAST_VALUE()`: FIRST\_VALUE 函数用于获取窗口内指定列的第一个值，LAST\_VALUE 函数用于获取窗口内指定列的最后一个值。
```sql
...
| exam\_id | duration | release\_time |
| --- | --- | --- |
...
| uid | days\_window | avg\_exam\_cnt |
| --- | --- | --- |
| 1006 | 6 | 2.57 |
...
| uid | start\_month | total\_cnt | complete\_cnt |
| --- | --- | --- | --- |
| 1002 | 202002 | 3 | 1 |
| 1002 | 202003 | 2 | 1 |
| 1002 | 202005 | 2 | 1 |
...
| uid | incomplete\_cnt | total\_cnt | incomplete\_rate |
| --- | --- | --- | --- |
| 1001 | 3 | 7 | 0.4286 |
| 1002 | 4 | 8 | 0.5000 |
| 1003 | 1 | 1 | 1.0000 |
...
| tag | exam\_cnt\_20 | exam\_cnt\_21 | growth\_rate | exam\_cnt\_rank\_20 | exam\_cnt\_rank\_21 | rank\_delta |
| --- | --- | --- | --- | --- | --- | --- |
| SQL | 3 | 2 | \-33.3% | 1 | 2 | 1 |
...
| tag | start\_year | exam\_cnt | exam\_cnt\_rank |
| --- | --- | --- | --- |
| C++ | 2020 | 3 | 1 |
| SQL | 2020 | 3 | 1 |
| PYTHON | 2020 | 2 | 3 |
...
SELECT
  ...
  concat(
    round(
 * (exam_cnt_21 - exam_cnt_20) / exam_cnt_20,
      1
    ),
    '%'
  ) AS growth_rate,
...
1. 将高难度的试卷，按每类试卷的得分，利用 max/min (col) over()窗口函数求得各组内最大最小值，然后进行归一化公式计算，缩放区间为\[0,100\]，即 min\_max\*100
2. 若某类试卷只有一个得分，则无需使用归一化公式，因只有一个分 max\_score=min\_score,score，公式后结果可能会变成 0。
3. 最后结果按 uid、exam\_id 分组求归一化后均值，score 为 NULL 的要过滤掉。
...
| exam\_id | start\_month | month\_cnt | cum\_exam\_cnt |
| --- | --- | --- | --- |
| 9001 | 202001 | 2 | 2 |
| 9001 | 202002 | 1 | 3 |
| 9001 | 202003 | 3 | 6 |
...
| start\_month | mau | month\_add\_uv | max\_month\_add\_uv | cum\_sum\_uv |
| --- | --- | --- | --- | --- |
| 202001 | 2 | 2 | 2 | 2 |
| 202002 | 4 | 2 | 2 | 4 |
| 202003 | 3 | 0 | 2 | 4 |
