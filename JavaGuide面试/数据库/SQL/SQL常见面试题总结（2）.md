---
title: "SQL常见面试题总结（2）"
source: "https://javaguide.cn/database/sql/sql-questions-02.html"
author:
  - "[[Guide]]"
published: 2026-05-18
created: 2026-07-06
description: "SQL常见面试题总结第二篇，详解INSERT、UPDATE、DELETE等DML数据操作语句，包括批量插入、从其他表导入、带更新的插入等实战技巧。重点围绕 增删改操作、表与索引操作、写在最后、数据库基础、SQL 等内容展开。结合 JavaGuide 知识体系梳理 数据库 / SQL / Sql Questions 0。"
tags:
  - "clippings"
---
...
| --- | --- | --- | --- | --- | --- | --- |
| id | int(11) | NO | PRI | auto\_increment | (NULL) | 自增 ID |
| uid | int(11) | NO |  |  | (NULL) | 用户 ID |
| exam\_id | int(11) | NO |  |  | (NULL) | 试卷 ID |
| start\_time | datetime | NO |  |  | (NULL) | 开始时间 |
| submit\_time | datetime | YES |  |  | (NULL) | 提交时间 |
...
请把 **examination\_info** 表中 `tag` 为 `PYTHON` 的 `tag` 字段全部修改为 `Python` 。
...
**描述** ：现有一张试卷作答记录表 exam\_record，其中包含多年来的用户作答试卷记录，结构如下表：作答记录表 `exam_record` ： **`submit_time`** 为 完成时间 （注意这句话）
...
**描述** ：现有一张试卷作答记录表 exam\_record，其中包含多年来的用户作答试卷记录，结构如下表：
...
| --- | --- | --- | --- | --- | --- | --- |
| id | int(11) | NO | PRI | (NULL) | auto\_increment | 自增 ID |
| uid | int(11) | NO | UNI | (NULL) |  | 用户 ID |
| nick\_name | varchar(64) | YES |  | (NULL) |  | 昵称 |
| achievement | int(11) | YES |  | 0 |  | 成就值 |
| level | int(11) | YES |  | (NULL) |  | 用户等级 |
| job | varchar(32) | YES |  | (NULL) |  | 职业方向 |
| register\_time | datetime | YES |  | CURRENT\_TIMESTAMP |  | 注册时间 |

作为数据分析师，请 **创建一张优质用户信息表 user\_info\_vip** ，表结构和用户信息表一致。
...
| examination\_info | 0 | PRIMARY | 1 | id | A | 0 |  |  |  | BTREE |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| examination\_info | 0 | uniq\_idx\_exam\_id | 1 | exam\_id | A | 0 |  |  | YES | BTREE |
| examination\_info | 1 | idx\_duration | 1 | duration | A | 0 |  |  |  | BTREE |
| examination\_info | 1 | full\_idx\_tag | 1 | tag |  | 0 |  |  | YES | FULLTEXT |
...
**描述** ：请删除 `examination_info` 表上的唯一索引 uniq\_idx\_exam\_id 和全文索引 full\_idx\_tag。
