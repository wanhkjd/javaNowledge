...
source: "https://javaguide.cn/distributed-system/protocol/paxos-algorithm.html"
author:
  - "[[Guide]]"
published: 2026-05-25
created: 2026-07-06
description: "Paxos 算法详解，讲解 Proposer、Acceptor、Learner 三类角色，Basic Paxos 两阶段流程、Multi-Paxos 优化、算法难点和与 Raft 算法的对比。重点围绕 背景、介绍、Basic Paxos 算法、Multi-Paxos 思想、分布式协议与算法 等内容展开。"
tags:
  - "clippings"
---
## 背景
...
- 若 n > 该 Acceptor 见过的最大提案编号 max\_n
	- 返回 `Promise(n, max_v)` ，其中 max\_v 是之前接受过的最大编号提案的值（若有）
		- 承诺不再接受编号 < n 的提案
- 若 n ≤ max\_n
	- 拒绝或忽略该请求
...
当 Proposer 收到超过半数 Acceptor 的 Promise 响应后，选择响应中 max\_v 最大的值（若无则任意选择一个值），向超过半数的 Acceptor 发送 `Accept(n, v)` 请求。

**Acceptor 的处理逻辑** ：

- 若 n ≥ 该 Acceptor 在 Phase 1 承诺的 max\_n
...
- 分布式系统中的一致性与共识算法： [http://www.xuyasong.com/?p=1970](http://www.xuyasong.com/?p=1970)
